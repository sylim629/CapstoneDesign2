...
public class DayView extends JPanel {
	...
	public DayView() {
		...
		this.setBorder(new MatteBorder(0, 0, 1, 0, new Color(0xA39483)));
	...
	private void updateLayout() {
		...
		if (lastDimension != null) {
			scheduleBox.setPreferredSize(new Dimension(lastDimension.width - 20, lastDimension.height - 35));
			dayLabel.setPreferredSize(
					new Dimension(dayLabel.getFontMetrics(dayLabel.getFont()).stringWidth(dayLabel.getText()), 15));

			int widthCount = (int) Math.ceil(lastDimension.width / ((lastDimension.width - 40)));
			int maxCount = scheduleBox.getPreferredSize().height / 12 * widthCount;

			for (int i = 0; i < maxCount && i < scheduleList.size(); i++) {
				JPanel scPanel = scheduleList.get(i);
				scPanel.setPreferredSize(new Dimension(lastDimension.width - 40, 6));
				scPanel.setVisible(true);

				if (isFocused)
					scPanel.setBackground(new Color(0xFFCCC0));
				else
					scPanel.setBackground(new Color(0xDD9787));
			}
		...
		if (isFocused) {
			setBackground(new Color(0xFFE5DF));
			dayLabel.setForeground(new Color(0x45331F));
			dayLabel.setFont(new Font("THEJung150", 0, 14));
		} else {
			setBackground(new Color(0x7A6C5D));
			dayLabel.setForeground(Color.WHITE);
			dayLabel.setFont(new Font("THEJung130", 0, 14));
		}

		if (prevMonth || nextMonth) {
			dayLabel.setForeground(new Color(0xA39483));
		...
		if (isToday)
			bottomBox.setBackground(new Color(0xDD9787));
		else
	...
	private void reloadSchedule() {
		...
		Date endDate = null;
		try {
			firstDate = formatter.parse(String.format("%04d%02d%02d 00:00:00", yearNumber, monthNumber, dayNumber));
			endDate = formatter.parse(String.format("%04d%02d%02d 23:59:59", yearNumber, monthNumber, dayNumber));
		} catch (ParseException e) {
			e.printStackTrace();
		}

		ArrayList<Schedule> scList = ScheduleManager.sharedInstance().getSchedules(firstDate, endDate);
